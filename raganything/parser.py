...
def main():
    ...
    if args.check:
        ...
        if doc_parser.check_installation():
            logger.info(f"✅ {args.parser.title()} is properly installed")
            return 0
        else:
            logger.error(f"❌ {args.parser.title()} installation check failed")
            return 1
    ...
    try:
        ...
        logger.info(f"✅ Successfully parsed: {args.file_path}")
