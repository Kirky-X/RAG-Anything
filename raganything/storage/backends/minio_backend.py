...
class MinIOStorageBackend(StorageBackend):
    ...
    async def store_file(self, file_path: str, content: bytes, metadata: Dict) -> str:
        ...
        s3_metadata = {"custom-data-b64": meta_b64}
    ...
    async def retrieve_file(self, file_id: str) -> bytes:
        ...
        try:
            response = await loop.run_in_executor(
                None, lambda: self.client.get_object(self.bucket_name, file_id)
            )
            try:
    ...
    async def delete_file(self, file_id: str) -> bool:
        ...
        try:
            await loop.run_in_executor(
                None, lambda: self.client.remove_object(self.bucket_name, file_id)
            )
            return True
        except Exception:
            return False
    ...
    async def get_metadata(self, file_id: str) -> Dict:
        ...
        try:
            stat = await loop.run_in_executor(
                None, lambda: self.client.stat_object(self.bucket_name, file_id)
            )
    ...
    async def update_metadata(self, file_id: str, metadata: Dict) -> bool:
        ...
        new_s3_metadata = {"custom-data-b64": meta_b64}
