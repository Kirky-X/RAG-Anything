...
- **🎤 智能音频处理** - 基于 `SenseVoiceSmall` 模型的高精度语音转文本，支持17+种音频格式
- **⚙️ 资源感知管理** - 自动检测 GPU/MPS/CPU 资源，实现模型智能调度与优化
...
RAG-Anything 采用高度模块化的架构进行解析器和模型管理。
- **音频解析器**：基于 `SenseVoiceSmall` 的高性能语音识别，支持 `mp3`, `wav`, `flac` 等多种格式。
- **模型管理器**：基于 `modelscope` 的统一模型管理，支持自动下载和缓存。
- **设备管理器**：智能资源分配策略 (CUDA > MPS > CPU)。

> 更多关于解析器和模型配置的详细信息，请参阅 [解析器与模型文档](docs/parsers_and_models.md)。
